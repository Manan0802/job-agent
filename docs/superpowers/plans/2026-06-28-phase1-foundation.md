...
> **Note (lazy engine):** the engine is resolved per‑call from `DB_PATH`, not bound once at import.
> This keeps each test's temp DB isolated (a `monkeypatch.setenv("DB_PATH", ...)` actually takes
> effect) and prevents the "duplicate id=1 on second `pytest` run" `IntegrityError`.
